...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration > 15000:
            ...
            L_distortion = distortion_map.mean()
        else:
            L_distortion = 0

        if iteration > 15000:
            depth_normal, _ = depth_to_normal(viewpoint_cam, depth_map[None, ...])
            depth_normal = depth_normal.permute(2, 0, 1)
            render_normal = torch.nn.functional.normalize(normal_map, p=2, dim=0)
            ...
            L_consistency = normal_error.mean()
        else:
            L_consistency = 0

        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + opt.depth_distortion * L_distortion + opt.normal_consistency * L_consistency
        loss.backward()
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    is_prune = True if iteration % opt.prune_interval == 0 else False
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.2, scene.cameras_extent, size_threshold, is_prune)
